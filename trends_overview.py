...
# Program creates CSV file with an overview about all Senates and all policy areas and their contribution to cross-party cosponsorship
...
for num in RelSen:
    ...
    with open(f'senate_{num}.json','r') as file: # Senate files (have to be of particular form and name)
        obj = json.load(file)
    ...
    try:
        threshold = np.quantile(same_set, alpha) # Determining threshold for significancy of cosponsorings
    except IndexError:
        threshold = "NaN"
        cross = "NaN"    
        d_rel = "NaN"  
            ...
            for po in rel_cross:
                ...
                if G.has_edge(k,l) == True:
                    ...
                    if exi == False:
                        ...
                        mydata = {'sen': num, 'spon': k, 'cospon': l, "SName": G.nodes[k]["name"], "CName": G.nodes[l]["name"], 'sponP': G.nodes[k]["party"], 'cosponP': G.nodes[l]["party"], 'data': arr}
                        piv = PolDict[PA]
...
for n in RelSen:
    ...
    for date in data:
        if date['sen']==n:
            for cou in range(0,32):
                total = total + date['data'][cou]           
    for cou in range(0,32):
        a_count = 0
        for dat in data:
            if dat['sen']==n:
                a_count = a_count + dat['data'][cou]

        poArea = PolArea[cou]
        lildict = {'senate': n, 'area': poArea , 'number': a_count, 'total': total, 'percent': a_count / total}
        cooldata.append(lildict)
    
# Writing results to CSV
        
header = ["Senate", "Area", "Cosponsorings", "Total", "Percent Contribution"]                           
with open('signPAnew.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)

    writer.writerow(header)
    for dax in cooldata:
        wdata=[]
        wdata.append(dax['senate'])
        wdata.append(dax['area'])
        wdata.append(dax['number'])
        wdata.append(dax['total'])
        wdata.append(dax['percent'])
